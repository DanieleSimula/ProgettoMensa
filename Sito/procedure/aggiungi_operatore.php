...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $nomeutente = $_POST['nome_operatore'];
    $email = $_POST['email_operatore'];
    $password_chiara = $_POST['password_operatore'];

//blocco try catch per gestire errori
try{
    if (empty($nomeutente) || empty($email) || empty($password_chiara)) {
    throw new Exception("Campi obbligatori mancanti");
}

$password_hash = password_hash($password_chiara, PASSWORD_DEFAULT);

$sql= "CALL aggiungi_operatore(?, ?, ?)";
...
// SUCCESSO
header("Location: ../operatore/operatori.php?msg=success_inserimento_operatore");
exit();       

}catch(Exception $e){
    $error_message = $e->getMessage();
    header("Location: ../operatore/operatori.php?msg=error_inserimento_operatore&error=" . urlencode($error_message));
    exit();
}
}
?>
